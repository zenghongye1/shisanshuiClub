...
//##Ter1128竖屏begin######################
@interface UniWebViewController : UIViewController

@property(nonatomic, assign) BOOL isVisible;
@property(nonatomic, retain) id exitBlock;

-(void)dismissViewControllerAnimated:(BOOL)_anim;

@end

@implementation UniWebViewController
...
-(void)onBackBtn:(UIButton *)button {
    [self dismissViewControllerAnimated:YES];
}
-(void)dismissViewControllerAnimated:(BOOL)_anim {
    [self dismissViewControllerAnimated:_anim completion:^{
        self.isVisible = NO;
        
        if (self.exitBlock) {
            void(^myBlock)() = self.exitBlock;
            myBlock();
            self.exitBlock = nil;
        }
    }];
}
-(void)exitBlock:(void(^)())_block {
    self.exitBlock = _block;
}
...
@property (nonatomic, retain) UniWebViewController *baseViewController;
...
- (void)setBaseViewController:(id)_viewController portrait:(BOOL)_isPortrait;
...
-(id) initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _toolBar = ({
            ...
            toolBar.translucent = NO; //关闭强制半透明
            toolBar.alpha = 0.8;
...
-(void)changeToInsets:(UIEdgeInsets)insets targetOrientation:(ScreenOrientation)orientation {
    UIView *unityView = [self baseViewController].view;
    ...
    if (orientation == landscapeLeft || orientation == landscapeRight) {
        if (BELOW_IOS_8) {
            ...
            self.toolBar.frame = CGRectMake(-edgeOffsetX, unityView.frame.size.width - 44, unityView.frame.size.height +edgeOffsetX*2, 44);
            self.spinner.frame = CGRectMake(unityView.frame.size.height / 2 - 65, unityView.frame.size.width / 2 - 65, 130, 130);
        } else {
            self.toolBar.frame = CGRectMake(-edgeOffsetX, unityView.frame.size.height - 44, unityView.frame.size.width +edgeOffsetX*2, 44);
    ...
    CGRect f = CGRectMake(viewRect.size.width/1280*insets.left,
                          viewRect.size.height/720*insets.top,
                          viewRect.size.width -viewRect.size.width/1280*( insets.left +insets.right),
                          viewRect.size.height -viewRect.size.height/720*( insets.top + insets.bottom)); 
    self.frame = f;
    self.insets = insets;
}
...
- (void)setBaseViewController:(id)_viewController portrait:(BOOL)_isPortrait {
    ...
    [self setBaseViewController:_viewController];
    [self setIsPortrait:_isPortrait];
}
...
-(void) addManagedWebViewName:(NSString *)name insets:(UIEdgeInsets)insets portrait:(BOOL)isPortrait {
    
    id baseViewController = nil;
    if (isPortrait) {
        // 竖屏处理
        baseViewController = [[UniWebViewController alloc] init];
    } else {
        baseViewController = UnityGetGLViewController();
    }
    
    UIView *unityView = ((UIViewController *)baseViewController).view;
    UniWebView *webView = [[UniWebView alloc] initWithFrame:unityView.frame];
    webView.mediaPlaybackRequiresUserAction = NO;
    
    if (isPortrait) {
        [webView setBaseViewController:baseViewController portrait:isPortrait];
    }
...
-(void) changeWebViewName:(NSString *)name insets:(UIEdgeInsets)insets {
    ...
    id curViewController = [self curViewController];
    if ([curViewController isKindOfClass:[UniWebViewController class]]
        && ![curViewController isEqual:[webView baseViewController]]
        && [(UniWebViewController *)curViewController isVisible]) {
        //防止横屏刷新
        return;
    }
...
-(void) changeWebView:(UniWebView *)webView insets:(UIEdgeInsets)insets {
    ...
    if ([webView isPortrait]) {
        id baseViewController = [webView baseViewController];
        if (baseViewController && [baseViewController isVisible] ==NO) {
            [baseViewController setIsVisible:YES];
            [UnityGetGLViewController() presentViewController:baseViewController animated:YES completion:^{
                //[webView changeToInsets:insets targetOrientation:UnityCurrentOrientation()];
            }];
        }
        [self setCurViewController:baseViewController];
    } else {
        [self setCurViewController:UnityGetGLViewController()];
    }
    [webView changeToInsets:insets targetOrientation:UnityCurrentOrientation()];
}
...
- (void)webViewDone:(UniWebView *)webView {
    ...
    id baseViewController = [webView baseViewController];
    if (baseViewController && [baseViewController isKindOfClass:[UniWebViewController class]]) {
        [baseViewController dismissViewControllerAnimated:YES];
    }
}
...
extern "C" {
    void _UniWebViewInit(const char *name, int top, int left, int bottom, int right, bool isPortrait);
...
void _UniWebViewInit(const char *name, int top, int left, int bottom, int right, bool isPortrait) {
    ...
    [[UniWebViewManager sharedManager] addManagedWebViewName:UniWebViewMakeNSString(name) insets:insets portrait:isPortrait];
}
...
int _UniWebViewScreenHeight() {
    UIViewController *curViewController = [[UniWebViewManager sharedManager] curViewController];
    
    if (BELOW_IOS_8) {
        if (UnityCurrentOrientation() == landscapeLeft || UnityCurrentOrientation() == landscapeRight) {
            return curViewController.view.frame.size.width;
        } else {
            return curViewController.view.frame.size.height;
        }
    } else {
        return curViewController.view.frame.size.height;
    }
