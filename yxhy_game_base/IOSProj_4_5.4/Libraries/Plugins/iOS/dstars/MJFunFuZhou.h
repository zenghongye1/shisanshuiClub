...
private:
	...
	static BOOL		CheckWinNew(CTiles & tilesHand, int nlaiziCount, CTiles & laiziCard, int ngamestyle);
	static BOOL CheckWinLaiZiNeed(CTiles &tilesHand, int nLaiZiCount, int nCardLength);

	static BOOL CheckWinLaiZiNeedNoJiang(CTiles &tilesHand, int nLaiZiCount, int nCardLength);	// 去掉将剩下的牌,可以有癞子
