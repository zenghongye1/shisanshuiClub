   ...
   if isHadGangFen then
        ...
        for j=1,maxPlayer do
          for k,v in ipairs(rewards[j].ganginfo) do
            local item_gang= {}
            --  ucFlag(18:直杠    19:暗杠   20:补杠)
             if v.ucFlag == 18 or v.ucFlag == 20 then
               if v.Target == i then
                 item_gang.des = "被明杠"                        
               elseif v.Chair == i then
                 item_gang.des = "明杠"
               end           
             ...
             if item_gang.des and item_gang.des ~= "" then
               item_gang.money = v.stScore[i] * basescore 
               local index=nil
               if v.chair==i or v.Target==i then
                   index=j
               else
                   index=v.Target
               end  
               local otherviewSeat = self.gvblnFun(index)         
               local player = self.compPlayerMgr:GetPlayer(otherviewSeat)         
               item_gang.p = MahjongTools.GetPosDes(selfPlayer.index, player.index)
