...
function comp_mjDice:Play(val1,val2,cb,isLaterHide,laterHideTime,AllTime)
	if (not self.bAni) then
        ...
        self.AllTime = (AllTime or 1.5) * self:AnimSpd()
        self.midTime = self.AllTime * 0.5 * self:AnimSpd()
        self.passTime = 0
        self.bLaterHide = isLaterHide or true
        self.laterHideTime = laterHideTime or 4 * self:AnimSpd()
        local num1 = val1
        local num2 = val2
...
function comp_mjDice:Hide()
    if self.objSelf and not IsNil(self.objSelf) then
	   self.objSelf:SetActive(false)
    end
end
