...
function this.IsSeason(value)
    local flag = false
    if type(value)=="number" and value >=41 and value <=44 then
        flag = true
    end
    return flag
end

function this.IsFlower(value)
    local flag = false
    if type(value)=="number" and value >=45 and value <=48 then
        flag = true
    end
    return flag
end

local wind = {"东风","南风","西风","北风","红中","发财","白板"}
local sf = {"春","夏","秋","冬","梅","兰","菊","竹"}
function this.MahjongValueToChinese(value)
    if value >=1 and value <=9 then
        return this.NumberToChinese(value).."万"
    elseif value >=11 and value <=19 then
        return this.NumberToChinese(value-10).."条"
    elseif value >=21 and value <=29 then
        return this.NumberToChinese(value-20).."筒"
    elseif value >=31 and value <=37 then
        return wind[value-30]
    elseif value >=41 and value <=48 then
        return sf[value-40]
    else
        logError("错误的value"..value)
    end
end
...
function this.MahjongValueToMeshIndex1(value)
