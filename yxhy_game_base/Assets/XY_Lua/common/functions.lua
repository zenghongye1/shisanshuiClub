...
local resMgr = nil
local _assetBundleParams
function getAssetBundleParams(path, _type, assetType)
	if _assetBundleParams == nil then
		_assetBundleParams = AssetBundleParams.New(path, _type, assetType)
	else
		_assetBundleParams.path = path
		_assetBundleParams.type = _type
		_assetBundleParams.assetType = assetType or 0
	end
	return _assetBundleParams
end
...
function newNormalObjSync(path, _type, assetType)
	...
	local abp = getAssetBundleParams(path, _type, assetType)
...
function newSceneResidentMemoryObjSync(path, type)
	...
	local abp = getAssetBundleParams(path, type, 0)
...
function AddListener(obj,path,f, Sel)
    obj=obj or nil
    if obj==nil then
        Fatal("obj is nil")
        return
    ...
    if f~=nil and ui~=nil then
       addClickCallbackSelf(ui.gameObject,f, Sel)
    end 
    if ui==nil then
        Fatal("ui not find")
    end
...
function addDBClickCallbackSelf(go, callback)
	if (go ~= nil) then
		...
		logError("can not find gameObject")
	end
end
...
function ParseJsonStr(jsStr)
    ...
    -- local output
    -- if pcall(function() output = json.decode(jsStr) end) then
    -- 	return output
    -- else
    -- 	return {}
    -- end
...
--[[--
 * @Description: 检查table是否含有某个值  
 ]]
function IsTblIncludeValue(value, tbl)
...
--[[--
 * @Description: 删除table中相同的值，只用于数组  
 ]]
function RemoveSameInTable(tbl)
	local length = #tbl
	for i=length,1,-1 do
		for j=1,i do
			if tbl[i] == tbl[j] then
...
function RemoveAllValueFormTable(value,tbl)
	if value and tbl and type(tbl) == "table" then
		local index = #tbl
		while index > 0 do
...
--[[--
 * @Description: 检查两个int数组table是否完全相等  
 ]]
function IntArrayCheckSame(f_tbl,s_tbl)
	if f_tbl and s_tbl and type(f_tbl) == "table" and type(s_tbl) == "table" then
		if #f_tbl == #s_tbl then
			for i=1,#f_tbl do
				if f_tbl[i]~=s_tbl[i] then
					return false
				end
			end
			return true
