...
function Utils.utf8len(str)
    ...
    while currentIndex <= #str do
        ...
        currentIndex = currentIndex + Utils.charSize(char)
        len = len + 1
    end
    return len
end
...
-- numChars:    要截取的字符长度
function  Utils.utf8sub(str, startChar, numChar)
    local startIndex = 1
    --获取实际起始位置(以byte为单位)
    while startChar > 1 do   --startChar>1 定位到起始字符的前一个字符
        local char = string.byte(str, startIndex)
        startIndex = startIndex + Utils.charSize(char)
        startChar = startChar - 1
    end

    local endIndex = startIndex
    --numChar > 0 定位到结束字符的后一个字符的起始位置
    while numChar > 0 and endIndex <= #str do
        local char = string.byte(str, endIndex)
        endIndex = endIndex + Utils.charSize(char)
        numChar = numChar - 1
    end
    --endIndex 指向结束字符的后一个字符  减一, 指向结束字符的最后一个字节的位置
    return str:sub(startIndex, endIndex - 1)
end
...
function Utils.WorldPosToScreenPos( pos)
	local screenPos = Camera.main:WorldToScreenPoint(pos)
	return Utils.ScreenPosToUIPos(screenPos)
end

function Utils.ScreenPosToUIPos( screenPos )
	local w = Screen.width
	local h = Screen.height
	if (w / h) > (1280 / 720) then
		local DefaultWidth = w / h * 720
		screenPos.x = (screenPos.x / w) * DefaultWidth - DefaultWidth / 2
		screenPos.y = (screenPos.y / h) * 720 - 720 / 2
		screenPos.z = 0
	else
		local DefineHeight = h/w * 1280
		screenPos.x = (screenPos.x / w) * 1280 - 640
		screenPos.y = (screenPos.y / h) * DefineHeight - DefineHeight / 2
		screenPos.z = 0
	end
	return screenPos
end
...
function Utils.NGUIPosTo2DCameraPos(pos)
	local uiCam = GameObject.Find("uiroot_xy/Camera"):GetComponent(typeof(Camera))
	if uiCam == nil then
		uiCam = UICamera.currentCamera
	end
	local viewPointPos = uiCam:WorldToViewportPoint(pos)
	local worldPos = GameObject.Find("2D Camera"):GetComponent(typeof(Camera)):ViewportToWorldPoint(viewPointPos)
	return worldPos
end


function Utils.NGUIPosTo3DCameraPos(pos)
	local uiCam = GameObject.Find("uiroot_xy/Camera"):GetComponent(typeof(Camera))
	if uiCam == nil then
		uiCam = UICamera.currentCamera
	end
	local viewPointPos = uiCam:WorldToScreenPoint(pos)
	local ray = Camera.main:ScreenPointToRay(viewPointPos)
	local worldPos 

	local flag, hit = Physics.Raycast(ray, nil, 25) 
	if flag then
		worldPos = hit.point
	else
...
function Utils.TwoDCameraTo3DPosAndScreenPos(camera, pos)
	local viewPointPos = camera:WorldToScreenPoint(pos)
	local ray = Camera.main:ScreenPointToRay(viewPointPos)
	local worldPos 
...
function Utils.ScreenPosToWorldPos(camera, screenpos)
...
function Utils.CheckUIContainsPoint(rect, point)
	if rect == nil or rect.localCorners == nil then
		return false
	end
	local uiCam = GameObject.Find("uiroot_xy/Camera"):GetComponent(typeof(Camera))
	if uiCam == nil then
	...
	wordPos = uiCam:ScreenToWorldPoint(point)
	local pos = rect.transform:InverseTransformPoint(wordPos)
	local corners = rect.localCorners
	if pos.x >= corners[0].x 
		and pos.x <= corners[2].x 
		and pos.y >= corners[0].y
		and pos.y <= corners[1].y then
		return true
	end
	return false
end

function Utils.CheckPointInUIs(rectList, point)
	for i = 1, #rectList do
		if Utils.CheckUIContainsPoint(rectList[i], point) then
			return true
		end
	end
	return false
...
function Utils.InsertSort(list)
 	for i = 2,#list,1 do
        local insertItem = list[i]
        local insertIndex = i - 1
        while (insertIndex > 0 and insertItem < list[insertIndex])
        do
...
local EffectComp = EffectComp
function Utils.SetEffectSortLayer(go, layer)
	if go == nil then
		return
	end
	local comp = go:GetComponent(typeof(EffectComp))
	if comp == nil then
		comp = go:AddComponent(typeof(EffectComp))
	end
	comp:SetSortOrder(layer)
end
