...
function comp_mjPlayer:ctor()
    self.comp_operatorcard_class = require "logic/mahjong_sys/components/mode_1/comp_mjOperCard"
...
function comp_mjPlayer:Init()
    ...
    self.handCardCount = 0
    self.showHuaInTableCardList = {}
    self.isShowHandCard = false
    self.ShowCardsNum = 0
    self.notShowCardsNum = 0
end
...
function comp_mjPlayer:ArrangeMjList(mjList, callback)
    ...
    local moveFunc = function(mj, down)
        ...
        local pos = mj.transform.localPosition
        if not down then
            mj:DOLocalMove(pos.x + mahjongConst.MahjongOffset_x * 2, y, nil, arrangeTime * self:AnimSpd(), false)
        else
            mj:DOLocalMove(nil, y, 0, arrangeTime * self:AnimSpd(), false)
        end
    end
    ...
    local ArrangeMjList_c = coroutine.start(function()
                ...
                -- 移动到牌的位置
                for i, v in ipairs(mjList) do 
                    mjList[i]:DOLocalMove(self:GetHandPosX(self.handCardCount + i, self.cfg.MahjongHandCount), 0, 0, arrangeTime*2 * self:AnimSpd(), false)
                end
...
function comp_mjPlayer:RemoveChangeFlowers( flowerCards, flowerCount, isDeal)
    ...
    if isDeal then
        ...
        for i = 1, #mjList do 
            ...
            mjList[i]:DOLocalMove(nil, nil, pos.z, 0.2 * self:AnimSpd())
        end
...
function comp_mjPlayer:DoOutCard(item, callback)
    self.DoOutCard_c = coroutine.start(function ()
        ...
        local x,y,z = endPos:Get()
         mj:DOLocalMove(x,y,z, 0.1 * self:AnimSpd(),false):OnComplete(function ()
            if callback ~= nil then
...
function comp_mjPlayer:PutFlowerMjList(mjList, needAnim, needClear)
    ...
    -- 设置状态
        ...
        for i = 1, #mjList do
            ...
            if needAnim then
                local pos = mjList[i].transform.localPosition
                mjList[i]:DOLocalMove(nil, nil, pos.z + 0.3, 0.5 * self:AnimSpd())
            ...
            * mjList[i]:GetWidth() + 0.5 * mjList[i]:GetWidth()
            ...
            mjList[i]:DOLocalMove(x,y,z, waitTime * self:AnimSpd())
...
function comp_mjPlayer:OperateCard( operData,mj )
    ...
    if operData.operType == MahjongOperAllEnum.FengGang then
        self:CreateOperCard(operData)
    end
...
--[[--
 * @Description: 设置不同数量手牌亮或者倒，其余保持手牌状态  
 ]]
function comp_mjPlayer:SetShowAndNotShowCards(showCards, notShowCardsNum, callback)
    self.isShowHandCard = true
    self.ShowCardsNum = #showCards
    self.notShowCardsNum = notShowCardsNum
    table.sort(showCards)
    self:ResetHandCardWithTing()
    ...
    for i = 1, #self.handCardList do
        if i <= #showCards then
            self.handCardList[i]:SetMesh(showCards[i])
            self.handCardList[i]:SetState(MahjongItemState.inOtherHand)
            self.handCardList[i]:DOLocalMove(self.handCardList[i].transform.localPosition.x,0,z, 0)
            self.handCardList[i]:DOLocalRotate(90,0,0, 0)
            self.handCardList[i]:ShowShadow()
        elseif i <= (#showCards + notShowCardsNum) and i > #showCards then
            self.handCardList[i]:SetState(MahjongItemState.inOtherHand)
            self.handCardList[i]:DOLocalMove(self.handCardList[i].transform.localPosition.x,0,z, 0)
            self.handCardList[i]:DOLocalRotate(-90,0,0, 0)
            self.handCardList[i]:ShowShadow()
...
function comp_mjPlayer:GetOperCardList( operData,mj )
    ...
    for i=1,#searchCard do
        ...
        if self.viewSeat == 1 then
            ...
            local isFind = false
            if self.isShowHandCard then
                local mjItem
                mjItem,isFind = self:FindHandCardIfShow(searchCard[i])
                if isFind then
                    table.insert(list,mjItem)
                end
            end
    ...
    for i=1,#self.operCardList do
        if (self.operCardList[i].operData.operType == MahjongOperAllEnum.TripletLeft or
            ...
            if self.viewSeat == 1 then
                ...
                if not isFind then
                    index = math.random(1,#self.handCardList)
                    mj = self.handCardList[index]
                    mj:SetMesh(operData.operCard)
                    table.remove(self.handCardList,index)
                end
...
function comp_mjPlayer:FindHandCardIfShow(cardValue)
    local mj
    local isFind = false
    local index = 1
    while(index<=#self.handCardList) do
        if(self.handCardList[index].paiValue == cardValue) then
            mj = self.handCardList[index]
            table.remove(self.handCardList,index)
            isFind = true
            break
    ...
    if not isFind and self.notShowCardsNum > 0 then
        local index = self.ShowCardsNum + 1
        mj = self.handCardList[index]
        mj:SetMesh(cardValue)
        table.remove(self.handCardList,index)
        isFind = true
        self.notShowCardsNum = self.notShowCardsNum - 1
    end
    return mj,isFind
end
...
function comp_mjPlayer:ResetOperCard(GetResetCardsFunc,operData)
    for i=1,#operData do
        ...
        elseif ucFlag == 20 then    
            ...
            oper:ReShow(od, cards)
        -- 亮喜儿
        elseif ucFlag == 9 then    
            operType = MahjongOperAllEnum.LiangXiEr
            local od = operatordata:New(operType,nil,{35,36,37}, ucFlag, operWho)
            local cards = GetResetCardsFunc(3)
            cards[1]:SetMesh(35)
            roomdata_center.AddMj(35)
            cards[2]:SetMesh(36)
            roomdata_center.AddMj(36)
            cards[3]:SetMesh(37)
            roomdata_center.AddMj(37)
            oper:ReShow(od, cards)
        else
            logError("not define ucFlag "..tostring(GetTblData(data)))
        end
