...
function comp_mjResMgr:LoadSpecialCardSign(specialCardResName)
    ...
    local tex = newNormalObjSync(mahjong_path_mgr.commonMJPrefix.."/texture/"..specialCardResName, typeof(UnityEngine.Texture))
    return UnityEngine.Sprite.Create(tex, UnityEngine.Rect.New(0,0,44,44), Vector2(0.5,0.5))
end
 
function comp_mjResMgr:GetSpecialCardSprite()
    local specialCardResName = self.mode.cfg.specialCardResName
    if self.SpecialCardSignMap[specialCardResName] == nil then
        self.SpecialCardSignMap[specialCardResName] = self:LoadSpecialCardSign(specialCardResName)
    end
    return self.SpecialCardSignMap[specialCardResName]
end
