    ...
    if mj == self.curClickMJItem then
        if self.canOutCard then
            ...
            if self:CheckCanSendOut(paiVal) then
                ...
                Trace("mahjong_play_sys.OutCardReq(paiVal,self.nbzz)----===================-------"..roomdata_center.nbzz)
                 mahjong_play_sys.OutCardReq(paiVal,roomdata_center.tingType,roomdata_center.kouCardList,roomdata_center.nbzz) 
                --边砸钻时候恢复灰质手牌
                if roomdata_center.nbzz and tonumber(roomdata_center.nbzz) ~= 0 then
