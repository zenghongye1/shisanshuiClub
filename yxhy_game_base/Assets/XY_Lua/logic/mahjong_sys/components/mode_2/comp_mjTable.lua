...
function comp_mjTable:ShowWallInternal(dunCallback, itemCallback)
    local index = 1
    local x,y,z
    local viewseat = player_seat_mgr.GetViewSeatByLogicSeat("p1")  --默认取p1位置    
    self.config:SetWallDunCount(roomdata_center.CheckWindCard(), viewseat)
    ...
    for i = 1, #wallCounts do   -- 四排
        ...
        for j = 1, wallCounts[i] do  -- 墩数
            for k = 1, 2 do     -- 每墩两个
                ...
                if self.cfg.isShowWall then
                	item:DOLocalMove(Vector3(x,y,0), 0)
                end
                index = index + 1
...
function comp_mjTable:ShowLai2(dun, cardValue, isAnim, callback)
    Trace("ShowLai !!!!!!!!!!!!! dun2 "..dun.." cardValue "..cardValue)    
    local laiIndex = self.lastIndex
        ...
        if not IsNil(self.mjSelectObj) and isAnim then                 
            for i=1, dun do                                               
                ...
                if tempMJ ~= nil then
                    ...
                    self.mjSelectObj.transform.eulerAngles = Vector3.New(tempMJ.transform.eulerAngles.x, tempMJ.transform.eulerAngles.y, 0)
                    --self.mjSelectObj.transform.eulerAngles = self.mjSelectObj.transform.eulerAngles - Vector3(0, 0, -180)
                    coroutine.wait(0.2) 
                end
...
local InitShowCi_c = nil
function comp_mjTable:ShowCi(dun,cardValue,cardSp,isAnim,callback)     
    local laiIndex = self.lastIndex+(dun-1)*2
    if(laiIndex>self.config.MahjongTotalCount) then
        laiIndex = laiIndex - self.config.MahjongTotalCount
    end
    ...
    InitShowCi_c = coroutine.start(function ()  
        if not IsNil(self.mjSelectObj) and isAnim then                 
            for i=1, dun do                                               
                tempMJ = self.compMjItemMgr.mjItemList[tempIndex]  
                if tempMJ ~= nil then
        ...
        if isAnim then
            ...
            coroutine.wait(0.5)
            mj:Clone(mjEx)
            self:MoveMjTo3DCenter(mjEx, 0.3)
            coroutine.wait(0.3)
            --  播放定次动画
            mjEx:SetActive(false)   -- 3D隐藏
            local trans = mahjong_ui:ShowSpecialCard(cardSp,1,self.cfg.specialCardSpriteName)
            mahjong_effectMgr:PlayUIEffectById(20017,mahjong_ui.playerList[1].transform.parent)
            coroutine.wait(1)
            mahjong_ui:HideCiCard()   -- 隐藏UI
            mjEx:SetActive(true)   -- 3D显示
            self:MoveMjTo2DCenter(mjEx, 0, Vector3(0, 0, 0))
            mjEx:Set2DLayer()
            self:DoHideToPoint({mjEx}, mahjong_ui:GetHunHidePos(), 0.2, nil, true)  
        end 
        coroutine.wait(0.6)
        if callback~=nil then
            callback()
...
function comp_mjTable:SendCardFromLast(viewseat, paiValue,isDeal)
    ...
    if self.isNeedUpdateCi then
        self:UpdateCiAsLastCard()
    end
    if isExchange and originPos_exchangeJin then
        self.mjJin[1]:DOLocalMove(originPos_exchangeJin,0)
    elseif self.isNeedUpdateGlod then
...
--更新次牌位置
function comp_mjTable:UpdateCiAsLastCard()
    if self.mjCi == nil then
        return
    end
    if self.compMjItemMgr.mjItemList[self.lastIndex]~= self.mjCi then
        if self.lastIndex % 2 == 1 then
            self:UpdateCiAtLastIndex()
            return
        else
            -- 直接落下
            self.mjCi:DOLocalMove(Vector3(self.mjCi.transform.localPosition.x, 0, 0), 0)
        end
    end
    self.mjCi:ShowShadow()
end

function comp_mjTable:UpdateCiAtLastIndex()
    ...
    if self.mjCi.transform.parent ~= parent then
        self.mjCi.transform:SetParent(parent, false)
    end
    local x = 0
    local y = 0
    x = pos.x
    y = pos.y + mahjongConst.MahjongOffset_y
    self.mjCi:DOLocalMove(Vector3(x, pos.y + mahjongConst.MahjongOffset_y, pos.z), 0)
end
...
function comp_mjTable:DoHideToPoint(mjList, pos, time, callback, is2d)
    ...
    for i = 1, #mjList do
        ...
        mjList[i]:Hide()
        mjList[i].transform.localScale = Vector3.one
    end
    end)
