...
function comp_mjItem:DOLocalMove(pos, time, snap)
    self:HideShadow()
    snap = snap or false
    local isChange = self:SetAndCheckVector3Change(self.localPosition, pos.x,pos.y,pos.z)
...
function comp_mjItem:LocalMoveByKey(key, value, time)
    ...
    self:DOLocalMove(localPos, time)
end
...
function comp_mjItem:SetSpecialCard(isShow)
    ...
    if self.specialIconGo~=nil then
        ...
        if isShow then            
            local sprite = mode_manager.GetCurrentMode():GetComponent("comp_resMgr"):GetSpecialCardSprite()
            if self.hunObj_Sprite == nil then
                self.hunObj_Sprite = componentGet(self.specialIconGo.transform, "SpriteRenderer")
            end
            if self.hunObj_SpriteMain == nil or self.hunObj_SpriteMain ~= sprite then
                self.hunObj_Sprite.sprite = sprite
                self.hunObj_SpriteMain = sprite
            end
        end        
    end
end
...
function comp_mjItem:HideAndReset()
    if(not IsNil(self.mjObj)) then
        ...
        -- self:SetActive(false)
        self:DOLocalMove(Vector3(999,999,999),0)
        self.transform.localScale = Vector3.one * mahjongConst.MahjongScale
        self.isDrag = false
