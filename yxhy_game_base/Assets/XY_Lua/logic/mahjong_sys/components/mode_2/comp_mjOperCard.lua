...
function comp_mjOperCard:ctor(enableDown)
	...
	self.enableDown = enableDown or self.cfg.gang_show_type or false
...
function comp_mjOperCard:AddShow(operData, mj, isChangeToBright)
	...
	if not self.config.ming_gang_type then	
		-- 碰杠 不需要倒下
...
function comp_mjOperCard:SetMjPos(mj, offset)
	...
	mj:DOLocalMove(Vector3(x,0,z), 0)
...
function comp_mjOperCard:TripletCollectRefresh()
	...
	for i = start ,total, step do
		...
		offsetX = off
		self.itemList[i]:DOLocalRotate(nil,nil, 0, 0)
...
function comp_mjOperCard:QuadrupletRefresh()
	...
	if self.enableDown then
		...
		for i = start ,total, step do
			...
			self.width = self.width + width
			if self.operData.operType == MahjongOperAllEnum.DarkBar or self.operData.operType==MahjongOperAllEnum.FengGang  then
