...
function mahjong_action_gameAskBlock:Execute(tbl)
	...
	if bCanTing then
		...
		local clientTingType = MahjongOperTipsEnum.Ting
		if tbl._para.cardTingGroup.msgreply == 1 then
			clientTingType = MahjongOperTipsEnum.YingBao
		end
		local cardTingGroup = tbl._para.cardTingGroup
		local operData = operatorTipsData:New(clientTingType,cardTingGroup)
		operatorcachedata.AddOperTips(operData)
	end
