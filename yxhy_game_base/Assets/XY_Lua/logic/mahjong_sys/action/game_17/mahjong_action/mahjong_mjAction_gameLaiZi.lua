...
function mahjong_mjAction_gameLaiZi:Execute(tbl)
    ...
    local zhuangPlayer = self.compPlayerMgr:GetPlayer(roomdata_center.zhuang_viewSeat)
    local laiIndex = self.compTable.sendIndex
    ...
    coroutine.start(function ()
        ...
        mj:SetParent( zhuangPlayer.outCardPoint, true)
        mj.transform.localScale = Vector3.one
        -- 牌上移 防止穿过牌墩
        LuaHelper.SetTransformLocalY(mj.transform, 0.4)

        local endPos = zhuangPlayer:GetOutCardPos()
        mj:DOLocalMove(endPos, 0.2, false)
        mj:DOLocalRotate(0, 0, 0, 0, DG.Tweening.RotateMode.Fast)
        table.insert(zhuangPlayer.outCardList, mj)
        coroutine.wait(0.2)
        mj:ShowShadow()               
