...
function mahjong_action_gameAskBlock:Execute(tbl)
	...
	-- 亮喜
	local bCanLiangXiEr = tbl._para.bCanLiangXiEr
	if bCanLiangXiEr then
		local operData = operatorTipsData:New(MahjongOperTipsEnum.LiangXiEr,tbl._para.cardLiangXiEr)
		operatorcachedata.AddOperTips(operData)
	end

	if bCanCollect or bCanTriplet or bCanQuadruplet or bCanTing or bCanWin or bCanLiangXiEr then
