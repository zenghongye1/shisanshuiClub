...
function mahjong_mjAction_gameChangeFlower:Execute(tbl)
    ...
    mahjong_anim_state_control.ShowEffectState(MahjongGameAnimState.changeFlower, 
        function()
...
function mahjong_mjAction_gameChangeFlower:ChangeFlowerAndHide(viewSeat, flowerCards, totalCards, newCards, callback,isDeal)
    ...
    local changeFlower_c = coroutine.start(function()
        ...
        self.compPlayerMgr:GetPlayer(viewSeat):RemoveChangeFlowers(flowerCards,flowerCount, Ontable)
        if isDeal then
            coroutine.wait(0.3)
        else
            coroutine.wait(0.5)
