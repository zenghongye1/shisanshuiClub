...
function this.ApplyForReq(bStatus)
  majong_request_interface.ApplyForReq(sessionData["_chair"], bStatus)
end
...
function this.OutCardReq(cardValue,tingType,turnCard,nbzz)
    local playPara = {cards = cardValue,tingType = tingType,turnCard = turnCard,nbzz = nbzz}
...
function this.TingReq()
    majong_request_interface.TingReq()
end

-- 抢听
function this.QiangTingReq(cardValue)
    majong_request_interface.QiangTingReq(cardValue)
end

--碰牌
function this.TripletReq(cardValue,nbzz)
	--[[--
	 * @Description: [ 21, 21,21]   
	 ]]
   if roomdata_center.nbzz == 1 then
      local tbl = operatorcachedata.GetOpTipsTblByType(MahjongOperTipsEnum.N_ZA_FLAG)
       majong_request_interface.TripletReq(tbl,sessionData["_gt"], sessionData["_chair"], nbzz)
   else
...
--杠牌
function this.QuadrupletReq(cardValues,nbzz)
	--[[--
	 * @Description: [ 21, 21,21,21]   
	 ]]
    majong_request_interface.QuadrupletReq(cardValues,sessionData["_gt"],sessionData["_chair"],nbzz)
end
...
--亮喜儿
function this.LiangXiErReq(cardValues)
    majong_request_interface.LiangXiErReq(cardValues,sessionData["_gt"],sessionData["_chair"])
end
