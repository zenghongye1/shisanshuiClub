...
local niuniu_largeResult_data = class("niuniu_largeResult_data",base)

function niuniu_largeResult_data:ctor(data)
	base.ctor(self)
	self.result = {}
	self:ProcessData(data)
end

function niuniu_largeResult_data:ProcessData(data)
	--重写
	local result = data
	local players = {}
	...
	for k,v in pairs(result["user_list"]) do
		...
		if result["totallInfo"][k]["winfo"] then
			player["tList"] = {
				...
				[3] = {[1] = "牛牛:",[2] = tostring(result["totallInfo"][k]["winfo"]["nBullTenNums"]).."次"},
			...
			player["tList"] = {
				...
				[3] = {[1] = "牛牛:",[2] = tostring(result["totallInfo"][k]["nums"][3]).."次"},
...
return niuniu_largeResult_data