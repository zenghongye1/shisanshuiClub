...
local sangong_ui_controller = class("sangong_ui_controller",base)
...
function sangong_ui_controller:InitDataAndUIMgr()
	self.ui = UI_Manager:Instance():GetUiFormsInShowList("sangong_ui")
	self.data_manage = require("logic.poker_sys.sangong_sys.cmd_manage.sangong_data_manage"):GetInstance()
end

function sangong_ui_controller:InitTableComponent()
	...
	if playerRob ~= nil and tbl._para.sCurrStage == "robbanker" then
		for i,v in ipairs(playerRob) do
			...
			elseif v > 0 then
					...
					Trace("viewSeat :"..tostring(viewSeat).." "..v.."倍")
			end
		end
	end	
	...
	if multLogicState ~= nil  then	--玩家下注状态：-1还没操作，>0下注底分 得显示按钮跟手排
		for i,v in ipairs(multLogicState) do
			...
			if v > 0 then
				...
				Trace("viewSeat :"..tostring(viewSeat).." "..v.."分")
			end
		end
	end
		--玩家亮牌状态：-1没亮，1亮
