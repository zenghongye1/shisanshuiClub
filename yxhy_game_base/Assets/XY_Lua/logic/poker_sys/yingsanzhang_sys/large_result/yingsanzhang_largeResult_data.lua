...
local yingsanzhang_largeResult_data = class("yingsanzhang_largeResult_data",base)

function yingsanzhang_largeResult_data:ctor(data)
	base.ctor(self)
	self.result = {}
	self:ProcessData(data)
end

function yingsanzhang_largeResult_data:ProcessData(data)
	--重写
	local result = data
	local players = {}
	...
	for k,v in pairs(result["user_list"]) do
		...
		if result["totallInfo"][k]["winfo"] then
			player["tList"] = {
				...
				[2] = {[1] = "豹子:",[2] = tostring(result["totallInfo"][k]["winfo"]["nBaoZiNums"]).."次"},
				[3] = {[1] = "顺金:",[2] = tostring(result["totallInfo"][k]["winfo"]["nShunJinNums"]).."次"},
				[4] = {[1] = "金花:",[2] = tostring(result["totallInfo"][k]["winfo"]["nJinHuaNums"]).."次"},
			}
		else
			player["tList"] = {
				[1] = {[1] = "胜利:",[2] = tostring(result["totallInfo"][k]["nums"][1]).."次"},
				[2] = {[1] = "豹子:",[2] = tostring(result["totallInfo"][k]["nums"][2]).."次"},
				[3] = {[1] = "顺金:",[2] = tostring(result["totallInfo"][k]["nums"][3]).."次"},
				[4] = {[1] = "金花:",[2] = tostring(result["totallInfo"][k]["nums"][4]).."次"},
			}
		end
...
return yingsanzhang_largeResult_data