...
return WS_GetBufferedAmount(ImplementationId);
		...
		/// </summary>
		public int PingFrequency { get; set; }		/// <summary>
...
this.Protocol = protocol;
		...
		private void OnInternalRequestCallback(HTTPRequest req, HTTPResponse resp)
		{
            ...
            //HTTPManager.Logger.Information("Websocket", "OnInternalRequestCallback, reason={0}================== reason " + reason);
            if (OnError != null)
		...
		private void OnInternalRequestUpgraded(HTTPRequest req, HTTPResponse resp)
		{
			...
			Debugger.Log(string.Format("Upgraded Socket:{0}", InternalRequest.Uri));
		...
		public void Open()
		{
			...
			requestSent = true;
			LuaInterface.Debugger.Log(string.Format("Open Socket:{0}", InternalRequest.Uri));
#else
try
{
...
Debugger.Log(string.Format("Open Socket:{0}, {1}", ImplementationId, InternalRequest.Uri));
...
WS_Send_Binary(this.ImplementationId, buffer, (int)offset, (int)count);
		...
		public void Close()
		{
			ConnectionBase conn = HTTPManager.GetConnectionWith(InternalRequest);
			if (conn != null) {
				Debugger.Log (string.Format("conn.Abort: {0}", InternalRequest.Uri));
				conn.Abort (HTTPConnectionStates.TimedOut);
			} else {
				Debugger.Log (string.Format("no find conn.Abort: {0}", InternalRequest.Uri));
			}
#if (!UNITY_WEBGL || UNITY_EDITOR)
			if (webSocket ==null)
				return;
			webSocket.Close();
			Debugger.Log(string.Format("Close Socket:{0}", InternalRequest.Uri));
#else
Debugger.Log (string.Format("close Socket:{0}, uir:{1}", ImplementationId,InternalRequest.Uri)));
if(ImplementationId == 0)
return;
WebSockets.Remove(ImplementationId);
///WS_Close(ImplementationId, 1000, "Bye!");
try
{
WS_Release(ImplementationId);
}
catch(Exception ex)
{
HTTPManager.Logger.Exception("WebSocket===", "WS_Release", ex);
		...
		public void Close(UInt16 code, string message)
		{
			ConnectionBase conn = HTTPManager.GetConnectionWith(InternalRequest);
			if (conn != null) {
				Debugger.Log (string.Format("conn.Abort: %s", InternalRequest.Uri));
				conn.Abort (HTTPConnectionStates.TimedOut);
			} else {
				Debugger.Log (string.Format("no find conn.Abort: %s", InternalRequest.Uri));
			}
#if (!UNITY_WEBGL || UNITY_EDITOR)
			if (!IsOpen)
				return;
...
return;
WebSockets.Remove(ImplementationId);
WS_Close(ImplementationId, code, message);
try
{
WS_Release(ImplementationId);
}
...
{
WebSocket ws;
Debugger.Log (string.Format("OnOpenCallback Socket:{0}, uri:{1}", id,InternalRequest.Uri));
...
{
WebSocket ws;
Debugger.Log (string.Format("OnErrorCallback Socket:{0}:{1}, uri:{2}", id, error, InternalRequest.Uri));
...
{
WebSocket ws;
Debugger.Log (string.Format("OnErrorCallback Socket:{0}:{1}, uri:{2}", id, reason, InternalRequest.Uri));
if (WebSockets.TryGetValue(id, out ws))
{
WebSockets.Remove(id);
