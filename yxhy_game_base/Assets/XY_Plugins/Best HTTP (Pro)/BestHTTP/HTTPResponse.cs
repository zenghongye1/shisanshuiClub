...
namespace BestHTTP
{
    ...
    public class HTTPResponse : IDisposable
    {
        ...
        internal virtual bool Receive(int forceReadRawContentLength = -1, bool readPayloadData = true)
        {
            ...
            HTTPManager.Logger.Information("HTTPResponse", "Response::Receive=========================");
            ...
            {
                HTTPManager.Logger.Information("HTTPResponse", "Status Line NULL???=========================");
                if (!baseRequest.DisableRetry)
                    return false;
            ...
            HTTPManager.Logger.Information("HTTPResponse", "To Read Headers???=========================");
            //Read Headers
            ReadHeaders(Stream);

            IsUpgraded = StatusCode == 101 && (HasHeaderWithValue("connection", "upgrade") || HasHeader("upgrade"));

			HTTPManager.Logger.Information("HTTPResponse", "IsUpgraded={0}, readPayloadData={1}=========================IsUpgraded " +  IsUpgraded + "readPayloadData " +  readPayloadData);
        ...
        protected void ReadHeaders(Stream stream)
        {
			...
			HTTPManager.Logger.Information("HTTPResponse", "headers={0}==========================stream " + stream);
        }
