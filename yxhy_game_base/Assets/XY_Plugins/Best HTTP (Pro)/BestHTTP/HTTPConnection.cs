            ...
            void ThreadFunc(object param)
        {
            ...
            try
            {
                ...
                if (TryLoadAllFromCache()) {
                    HTTPManager.Logger.Information("HTTPConnection", "From Cached=========================");
                    return;
                }
#endif

                if (Client != null && !Client.IsConnected()){
                    HTTPManager.Logger.Information("HTTPConnection", "Not Connected=========================");
                    Close();
                }
                ...
                do // of while (reconnect)
                {
                    ...
                    if (sentRequest)
                    {
                        HTTPManager.Logger.Information("HTTPConnection", "To Received=========================");
                        bool received = Receive();

                        if (State == HTTPConnectionStates.TimedOut ||
                            State == HTTPConnectionStates.AbortRequested)
                        {
                            HTTPManager.Logger.Information("HTTPConnection", "Abort Exception=========================");
                            throw new Exception("AbortRequested");
                        }
                        ...
                        if (CurrentRequest.Response != null)
                        {
                            ...
                            HTTPManager.Logger.Information("HTTPConnection", "Response Not NULL. Status Code={0}=========================" + CurrentRequest.Response.StatusCode);
...
#endif
                    HTTPManager.Logger.Exception("HTTPConnection", "Exception...", e);
