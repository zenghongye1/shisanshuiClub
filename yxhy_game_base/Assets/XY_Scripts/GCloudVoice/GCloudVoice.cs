...
public abstract class IGCloudVoice
{
    ...
    /// Stop player's microphone and stop record player's voice.
    /// </summary>
    /// <param name="bOptimized">optimization flag, true means stop record and keep play and record mode, if this operation makes screen not fluent you can use it. false is same before.</param>
    ///<returns>if success return GCLOUD_VOICE_SUCC, failed return other errno @see GCloudVoiceErrno</returns>
    public abstract int StopRecording(bool bOptimized);
#endif
