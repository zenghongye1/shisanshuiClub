...
namespace XYHY.ABSystem
{
    ...
    public abstract class AssetBundleLoader : IComparable<AssetBundleLoader>
    {
        ...
        public virtual void Start(bool _isSync = false)
        {
            id = idCounter++;
        }
    ...
    public class MobileAssetBundleLoader : AssetBundleLoader
    {
        ...
        /// 开始加载
        /// </summary>
        override public void Start(bool _isSync = false)
        {
            isSync = _isSync;
            if (_hasError)
                state = LoadState.State_Error;

            //if (state == LoadState.State_Complete)
            //{
            //    if (bundleInfo != null && bundleInfo.bundle == null)
            //    {
            //        Debug.LogError(_assetBundleCachedFile);
            //    }
            //}
