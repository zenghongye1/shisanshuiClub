...
namespace XYHY.ABSystem
{
    ...
    {
        private FileChangeType fileChangeMode = FileChangeType.None;
        public FileChangeType FileChangeMode
        {
            get { return fileChangeMode; }
            set { fileChangeMode = value; }
        }
        ...
        public AssetTarget(Object o, FileInfo file, string assetPath)
        {
            ...
            string tempStr = assetPath.Substring(0, assetPath.LastIndexOf('.')).Replace("Assets\\Res_XYHY\\", "");
            string secondDirName = tempStr.Split('\\')[0];
            ...
            {
                for (int j = 0; j < dstFileNameLst.Length; ++j)
                {
                    string directoryName = "";
                    int index = dstFileNameLst[j].IndexOf("Res_XYHY");
                    if (index != -1)
                    {
                        directoryName = dstFileNameLst[j].Substring(index);
                    }
#if UNITY_IOS || UNITY_IPHONE
                        directoryName = directoryName.Split('/')[1];
#elif UNITY_ANDROID || (AB_WINDOW && UNITY_STANDALONE)
                    directoryName = dstFileNameLst[j].Split('\\')[1];
#endif
                    if (j == 0)
                    {
                        commDirName = directoryName;
                    }
                    if ( directoryName.Contains(secondDirName))
                    {
                        flag = true;
                        break;
                    }
            ...
            if (flag)
            {
                this.belongName = secondDirName;
            }
            else
            {
                this.belongName = commDirName;
            }

            if (assetPath.EndsWith(".lua.bytes") || assetPath.EndsWith(".bytes"))
            {
#if UNITY_ANDROID || (AB_WINDOW && UNITY_STANDALONE)
                this.assetPath = assetPath.Replace("Assets\\Resources\\", "");
#elif UNITY_IOS || UNITY_IPHONE
				this.assetPath = assetPath.Replace("Assets/Resources/", "");
#endif
                this.belongName = commDirName;
            ...
            {
                this.bundleName = HashUtil.Get(AssetBundleUtils.ConvertToABName(tempStr)) + ".ab";
        ...
        public void Analyze()
        {
            ...
            if (_cacheInfo == null)
            {
                fileChangeMode = FileChangeType.NewFile;
            }
            else if(_cacheInfo != null && _isFileChanged)
            {
                fileChangeMode = FileChangeType.FileChange;
            }
            ...
            string[] paths = AssetDatabase.GetDependencies(assetPath, false);
            ...
            for (int i = 0; i < paths.Length; ++i)
            {
                ...
                //不包含builtin对象
                if (paths[i].StartsWith("Resources"))
                    continue;
            ...
            {
                if (File.Exists(pathLst[i]) == false)
                {
                    //Debug.Log("invalid:" + paths[i]);
                    continue;
                }
                FileInfo fi = new FileInfo(pathLst[i]);
                AssetTarget target = AssetBundleUtils.Load(fi);
                if (target == null)
                    continue;
