        ...
        public static void SaveCache()
        {
            for(int i=0; i<AssetBundleBuildPanel.verConfLst.Count; i++)
            {
                ...
                foreach (AssetTarget target in _object2target.Values)
                {
                    ...
                    //                    string key = "";
                    //#endif
                    string key = "ver_" + target.belongName + ".txt";
...
//#endif
                    ...
                    string tempStr = item.assetPath.Substring(0, item.assetPath.LastIndexOf('.')).Replace("Assets\\Res_XYHY\\", "");
#elif UNITY_IOS || UNITY_IPHONE
                string tempStr = item.assetPath.Substring(0, item.assetPath.LastIndexOf('.')).Replace("Assets/Res_XYHY/", "");
#else
                string tempStr = "";
#endif  
                    string assetBundleName = item.belongName + "/" + HashUtil.Get(ConvertToABName(tempStr)) + ".ab";
