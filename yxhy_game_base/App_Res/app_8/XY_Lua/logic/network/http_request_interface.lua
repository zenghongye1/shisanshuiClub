...
function this.ErrorCodeHandler(dicID)
    ...
    fast_tip.Show(GetDictString(dicID), time)
end
...
function this.GetTable(method, param)
    ...
    local t={["appid"]= global_define.appId, ["uid"]=uid, ["session_key"]=session_key, ["siteid"]=siteid, ["version"]=_version, ["method"]=method,["deviceid"]=deviceid,["devicetype"]=devicetype,["param"]=param}   
    return t
end 
...
function  this.otherLogin(rtype,openid,access_token,logintime,subrtype,share_uid,callback,isQuit)
    ...
    this.HttpPOSTRequest(rt, function (str,code, msg)
        ...
        if code == 0 then
            ...
            if ret ~= 0 then
                if ret == 100 then
                    fast_tip.Show("此账号不允许登录，请联系客服")
                elseif ret == 999 then 
                    fast_tip.Show("账号已被封停，登录游戏失败")
                else
                    logError("ret code:".. ret)
                end
            else
...
function this.getRoomListByGid(param,callback)
    ...
    this.HttpPOSTRequest(rt,function (str)
        waiting_ui.Hide()
        if code == -1 then
            fast_tip.Show(GetDictString(6015))
        else 
            callback(str)
        end 
    end)
...
function this.ShowError(code,rt,callback,dontShowWaiting, needRetry, retryTime,isQuit)
    if code == -1 then
        waiting_ui.Hide()
        message_box.ShowGoldBox("网络请求失败，是否重新请求",{function ()message_box.Close()
             if isQuit then
                Application.Quit() 
             end
         end,
