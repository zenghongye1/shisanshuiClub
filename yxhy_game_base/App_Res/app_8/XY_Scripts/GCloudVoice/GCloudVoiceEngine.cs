...
namespace gcloud_voice
{
	public class GCloudVoiceEngine : IGCloudVoice
	{
		...
		[DllImport(LibName, CallingConvention = CallingConvention.Cdecl)]
		private static extern int GCloudVoice_StopRecording(bool bOptimized);
		[DllImport(LibName, CallingConvention = CallingConvention.Cdecl)]
		...
		public GCloudVoiceEngine()
		{
			...
			if(ret != 0)
			{                
				Debugger.Log("Create GCloudVoiceInstance failed!");
			}
		...
		public override int Init()
		{
			if(!bIsSetAppInfo)
			{
				Debugger.Log("please set appinfo first");
			...
			if(!bInit)
			{
				...
				if(ret != 0)
				{
					Debugger.Log("Init GCloudVoice failed!");
					return ret;
				}
				bInit = true;
			} 
		...
		{
			Debugger.Log("GCloudVoice_C# API: _SetMode");
			if (!bInit)
			{
		...
		public NoticeMessage NoticeMessage_ParseFrom(byte[] buf, int buflen)
		{
			...
			if (buflen - guard < sizeof(Int32)) {
				Debugger.Log("notifymsg,parse error, buf small then sizeof(int)");
				return null;
			}
		...
		{	
			...
			if (msg.what == (int)NoticeMessageType.MSG_ON_JOINROOM_COMPLETE) {
				if (OnJoinRoomComplete != null) {
					Debugger.Log ("c# poll callback OnJoinRoomComplete not null");
		...
		{
			Debugger.Log("GCloudVoice_C# API: _Pause");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: _Pause nRet=" + nRet);
			return nRet;
		}
		
		public  override int Resume()
		{
			Debugger.Log("GCloudVoice_C# API: _Resume");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: _Resume nRet=" + nRet);
			return nRet;
		}
		
		public override int JoinTeamRoom(string roomName, int msTimeout)
		{
			Debugger.Log("GCloudVoice_C# API: JoinTeamRoom");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: JoinTeamRoom  nRet=" + nRet);
			return nRet;
		}

		public override int JoinTeamRoom(string roomName, string token, int timestamp, int msTimeout)
		{
			Debugger.Log("GCloudVoice_C# API: JoinTeamRoom"+" mstimeout:"+msTimeout);
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
		...
		{
			Debugger.Log("GCloudVoice_C# API: JoinNationalRoom");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: JoinNationalRoom  nRet=" + nRet);
			return nRet;
		}

		public override int JoinFMRoom(string roomID,int msTimeout)
		{
			Debugger.Log("GCloudVoice_C# API: JoinFMID");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			//Debugger.Log("GCloudVoice_C# API: JoinFMRoom  nRet=" + nRet);
			return nRet;
		}		
		...
		{
			Debugger.Log("GCloudVoice_C# API: QuitRoom");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: QuitRoom  nRet=" + nRet);
			return nRet;
		}
		
		public override int OpenMic()
		{
			Debugger.Log("GCloudVoice_C# API: OpenMic");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: OpenMic  nRet=" + nRet);
			return nRet;
		}
		
		public override int CloseMic()
		{
			Debugger.Log("GCloudVoice_C# API: CloseMic");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: CloseMic  nRet=" + nRet);
			return nRet;
		}
		
		public override int OpenSpeaker()
		{
			Debugger.Log("GCloudVoice_C# API: OpenSpeaker");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: OpenSpeaker  nRet=" + nRet);
			return nRet;
		}
		
		public override int CloseSpeaker()
		{
			Debugger.Log("GCloudVoice_C# API: CloseSpeaker");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: CloseSpeaker  nRet=" + nRet);
			return nRet;
		}
		
		public override int ApplyMessageKey(int msTimeout)
		{
			Debugger.Log("GCloudVoice_C# API: ApplyMessageKey");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: ApplyMessageKey  nRet=" + nRet);
			return nRet;
		}
		...
		{
			Debugger.Log("GCloudVoice_C# API: SetMaxMessageLength");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: SetMaxMessageLength  nRet=" + nRet);
			return nRet;
		}
		
		public override int StartRecording(string filePath)
		{
			Debugger.Log("GCloudVoice_C# API: StartRecording");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: StartRecording  nRet=" + nRet);
			return nRet;
		}
		
		public override int StopRecording ()
		{
			Debugger.Log("GCloudVoice_C# API: StopRecording");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			
			int nRet = GCloudVoice_StopRecording(false);
			Debugger.Log("GCloudVoice_C# API: StopRecording  nRet=" + nRet);
			return nRet;
		}
		...
		public override int StopRecording (bool bOptimized)
		{
			Debugger.Log("GCloudVoice_C# API: StopRecording");
			if (!bInit)
			{
			...
			int nRet = GCloudVoice_StopRecording(bOptimized);
			Debugger.Log("GCloudVoice_C# API: StopRecording  nRet=" + nRet);
			return nRet;
		}
#endif
		...
		{
			Debugger.Log("GCloudVoice_C# API: UploadRecordedFile");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: UploadRecordedFile  nRet=" + nRet);
			return nRet;
		}	
		
		public override int DownloadRecordedFile(string fileID, string downloadFilePath, int msTimeout)
		{
			Debugger.Log("GCloudVoice_C# API: DownloadRecordedFile");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: DownloadRecordedFile  nRet=" + nRet);
			return nRet;
		}		
		
		public override int PlayRecordedFile(string downloadFilePath)
		{
			Debugger.Log("GCloudVoice_C# API: PlayRecordedFile");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: PlayRecordedFile  nRet=" + nRet);
			return nRet;
		}		
		
		public override int StopPlayFile()
		{
			Debugger.Log("GCloudVoice_C# API: StopPlayFile");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: StopPlayFile  nRet=" + nRet);
			return nRet;
		}	
		
		public override int SpeechToText(string fileID, int language = 0, int msTimeout = 6000)
		{
			Debugger.Log("GCloudVoice_C# API: SpeechToText");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: SpeechToText  nRet=" + nRet);
			return nRet;
		}	
		...
		{
			Debugger.Log("GCloudVoice_C# API: ForbidMemberVoice");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: ForbidMemberVoice  nRet=" + nRet);
			return nRet;
		}
		
		public override int EnableLog(bool enable)
		{
			Debugger.Log("GCloudVoice_C# API: EnableLog");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: EnableLog  nRet=" + nRet);
			return nRet;
		}
		
		public override int GetMicLevel()
		{
			Debugger.Log("GCloudVoice_C# API: GetMicLevel");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: GetMicLevel  nRet=" + nRet);
			return nRet;
		}
		...
		{
			Debugger.Log("GCloudVoice_C# API: GetSpeakerLevel");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: GetSpeakerLevel  nRet=" + nRet);
			return nRet;
		}
		
		public override int SetSpeakerVolume(int vol)
		{
			Debugger.Log("GCloudVoice_C# API: SetSpeakerVolume");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: SetSpeakerVolume  nRet=" + nRet);
			return nRet;
		}
		
		public override int TestMic()
		{
			Debugger.Log("GCloudVoice_C# API: TestMic");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: TestMic  nRet=" + nRet);
			return nRet;
		}
		
		public override int GetFileParam(string filepath, int [] bytes, float [] seconds)
		{
			Debugger.Log("GCloudVoice_C# API: GetFileParam");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: GetFileParam  nRet=" + nRet);
			return nRet;
		}
		
		public override int invoke( uint nCmd, uint nParam1, uint nParam2, uint [] pOutput )
		{
			Debugger.Log("GCloudVoice_C# API: invoke");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			Debugger.Log("GCloudVoice_C# API: invoke  nRet=" + nRet);
			return nRet;
		}
		public override int EnableSpeakerOn(bool bEnable)
		{
			Debugger.Log("GCloudVoice_C# API: EnableSpeakerOn");
			...
			Debugger.Log("GCloudVoice_C# API: GCloudVoice_EnableSpeakerOn  nRet=" + nRet);
			return nRet;
		...
		{
			Debugger.Log("GCloudVoice_C# API: SetMicVol");
			...
			Debugger.Log("GCloudVoice_C# API: GCloudVoice_SetMicVol  nRet=" + nRet);
			return nRet;
