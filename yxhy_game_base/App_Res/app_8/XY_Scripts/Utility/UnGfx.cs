...
public static class UnGfx
{
    ...
    public static void SetResolution(int height, bool flag = true)
    {
        ...
        //int width = (int)w;
        //width &= ~1;
        Screen.SetResolution(Screen.width, Screen.height, flag);        
    }

    public static void AdaptRootUI()
    {
        float w = 1280;
        float h = 720;
        float baesRatio = h / w;
        float curRatio = Screen.height / Screen.width;
        UIRoot root = GameObject.FindObjectOfType<UIRoot>();
        if (root != null)
        {
            if (curRatio > baesRatio)
            {
                root.manualHeight = Mathf.RoundToInt(w / Screen.width * h);
            }
            else
            {
                root.manualHeight = (int)h;
            }
        }
    }
