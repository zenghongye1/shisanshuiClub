...
{
    static private FastFilter s_instance;
    ...
    {
        public fixed Number FastCheck[CharCount * 3];
    }
    private Number* m_fastCheck;
    private Number* m_startLength;
    private Number* m_endLength;
    private FixedBuffer m_buffer = new FixedBuffer();
    unsafe public FastFilter()
    {
        fixed (Number* ptr = m_buffer.FastCheck)
        {
            m_fastCheck = ptr;
    ...
    private void Init()
    {
        TextAsset asset = Resources.Load<TextAsset>("BadWord");
